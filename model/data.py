...
class Graph():
    def __init__(self):
        ...
        self.pre_target_nodes: np.ndarray = np.array([])
        self.train_target_nodes: np.ndarray = np.array([])
        self.valid_target_nodes: np.ndarray = np.array([])
        self.test_target_nodes: np.ndarray = np.array([])
...
def sample_subgraph(graph, time_range, sampled_depth = 2, sampled_number = 8, inp = None, feature_extractor = feature_pyg,
                    return_prompt_index = False, prompt_node_feature: list = None, prompt_width: int = 0, augment = False):
    '''
    ...
    '''
    group_ids = defaultdict( #source_type
        lambda: defaultdict(  #source_id
            lambda: []
        ))
    ...
    def add_budget(te, target_id, target_time, layer_data, budget):
        for source_type in te:
            ...
            for relation_type in tes:
                ...
                if len(adl) < sampled_number or sampled_number < 0:
                    sampled_ids = list(adl.keys())
                else:
                    sampled_ids = np.random.choice(list(adl.keys()), sampled_number, replace = False)
                if augment and len(sampled_ids) >= 5:
                    sampled_ids = np.random.choice(sampled_ids, int(len(sampled_ids) * 0.2), replace=False)
                for source_id in sampled_ids:
                    ...
                    for gid, depth_ in group_ids[source_type][target_id]:
                        group_ids[source_type][source_id].append((gid, depth_ + 1))

    '''
        First adding the sampled nodes then updating budget.
    '''
    for _type in inp:
        for _i, (_id, _time) in enumerate(inp[_type]):
            layer_data[_type][_id] = [len(layer_data[_type]), _time]
            group_ids[_type][int(_id)] = [(_i, 0)]
    for _type in inp:
    ...
    for layer in range(sampled_depth):
        ...
        for source_type in sts:
            ...
            if sampled_number > len(keys) or sampled_number == -1:
                '''
    ...
    if augment:
        mask_index = np.random.random(len(feature['def'])) < 0.2
        feature['def'][mask_index] = np.mean(feature['def'], axis=0)
                ...
                for target_key in tld:
                    ...
                    for source_key in tesr[target_key]:
                        ...
                        if source_key in sld:
                            if augment and np.random.random() < 0.2:
                                continue
                            source_ser = sld[source_key][0]
                            edge_list[target_type][source_type][relation_type] += [[target_ser, source_ser]]

    prompt_index = 0
    if prompt_node_feature is not None:
        extra_feature = [feature['def']]
        for head_id in range(len(inp['def'])):
            extra_feature.extend(prompt_node_feature)
            prompt_index += 1
            for i in range(len(prompt_node_feature)):
                idx = head_id * len(prompt_node_feature) + len(feature['def']) + i
                edge_list['def']['def']['self'].append([idx, idx])
                times['def'] = np.concatenate([times['def'], [1]], axis=0)

        for i in range(len(feature['def'])):
            for group_id, group_depth in group_ids['def'][indxs['def'][i]]:
                if group_depth > prompt_width:
                    continue
                for j in range(len(prompt_node_feature)):
                    prompt_node_index = group_id * len(prompt_node_feature) + len(feature['def']) + j
                    edge_list['def']['def']['def'].append([prompt_node_index, i])
                    edge_list['def']['def']['def'].append([i, prompt_node_index])

        feature['def'] = np.concatenate(extra_feature, axis=0)

    if return_prompt_index:
        return feature, times, edge_list, indxs, texts, (prompt_index, inp['def'])
    return feature, times, edge_list, indxs, texts

def to_torch(feature, time, edge_list, graph, edge_type_extra=None):
    '''
    ...
    for ee in edge_type_extra:
        edge_dict[ee] = len(edge_dict)
